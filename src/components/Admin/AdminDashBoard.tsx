  ...
  const AdminDashBoard = () => {
    ...
    const renderPage = () => {
      switch (currentTab) {
        ...
        case "vendordetails": return <VendorDetails />;
        case "userorders": return <UserOrders />;
        case "approvals": return <VendorApproval />;
        case "productapprovals": return <ProductApproval />;
    ...
    const menu = [
      ...
      { id: "vendordetails", label: "Vendor Details", icon: <FaStore size={18} /> },
      { id: "userorders", label: "User Orders", icon: <FaShoppingBag size={18} /> },
      { id: "approvals", label: "Approvals", icon: <FaCheckCircle size={18} /> },
      { id: "productapprovals", label: "Products Request", icon: <FaBox size={18} /> }
    ]

    return (
      /* PERFECT FIX 1: Parent viewport core change. 
        Using a strict grid layout instead of flex rows ensures that the workspace 
        is divided seamlessly with ZERO dead spaces or side margins.
      */
      <div className='w-full min-h-screen bg-[#0a0a0c] text-white grid grid-cols-1 lg:grid-cols-[18rem_1fr] relative font-sans antialiased overflow-hidden'>

        {/* Ambient Cyber Glow Background Highlights */}
        <div className="absolute top-[-5%] left-[-5%] w-[50%] h-[50%] bg-[#0d0d11] blur-[150px] rounded-full pointer-events-none z-0" />
        <div className="absolute bottom-[-10%] right-[-5%] w-[45%] h-[50%] bg-violet-600/5 blur-[150px] rounded-full pointer-events-none z-0" />
        ...
        <motion.div
          ...
          className='hidden lg:flex flex-col bg-[#0d0d11] border-r border-white/5 p-6 pt-28.75 backdrop-blur-3xl min-h-screen sticky top-0 z-20 w-full'
        >
          <div className="mb-6 px-1.5">
            <span className="text-[9px] text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-widest">Root Directive</span>
            <p className='text-base font-extrabold text-white mt-1'>Admin Engine</p>
          </div>

          <div className='flex h-screen flex-col gap-1 relative w-full'>
            {menu.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  className={`relative flex items-center gap-3.5 w-full px-4 py-3 rounded-xl transition-colors duration-200 text-xs font-semibold tracking-wide outline-none group z-10
                    ${isActive ? "text-white" : "text-zinc-400 hover:text-white"}`}
                >
                  {isActive && (
                    <motion.div
                      layoutId="admin-sidebar-pill"
                      className="absolute inset-0 bg-linear-to-r from-red-500/10 to-violet-500/5 border border-red-500/15 rounded-xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.03)] -z-10"
                      transition={{ type: "spring", stiffness: 380, damping: 30 }}
                    />
                  )}
                  <span className={`transition-all duration-200 ${isActive ? "scale-105 text-red-400 drop-shadow-[0_0_8px_rgba(239,68,68,0.4)]" : "text-zinc-500 group-hover:text-zinc-300"}`}>
                    {item.icon}
                  </span>
        ...
        {/* --- Main Sub-view 100% Fluid Edge-to-Edge Canvas Area --- */}
        {/* PERFECT FIX 2: Fixed padding configurations and width rules.
          w-full combined with px-4 sm:px-6 ensures layout fills the screen perfectly right up to the border line.
        */}
        <div className='min-h-screen'>
          <div className='w-full flex-1 flex flex-col items-stretch pt-28'>
  ...
  // w-full min-h-screen px-4 sm:px-6 lg:px-8 pb-12 pt-[95px] lg:pt-[115px] overflow-y-auto z-10 flex flex-col items-stretch
